...
useEffect (() => {
  ...
  const saveItemToDevice = async () => {
   try {
    ...
    await AsyncStorage.setItem('RarenCheckList', itemJson);
   }catch (error) {
  ...
  const getItemsFromDevice = async () => {
    try {
      const items = await AsyncStorage.getItem('RarenCheckList')
      if (items != null) {
  ...
  const removeAll = () => {
    Alert.alert("Limpar Lista?", "Confirma a exclusãode todos os produtos de sua lista?",
    [{
      text: 'Sim', 
      onPress: () => { setItems([])}
    }, {
